...
const PROMPTPAY_TARGET = {
  mobile: "0812345678", // ← ตั้งเป็นของร้านคุณ
...
export default function CheckoutPage() {
  ...
  // ✅ ควบคุมการแสดงผลหลังอัปโหลดสลิปสำเร็จ
  ...
  useEffect(() => {
    ...
    if (saved?.order_no) {
      setOrder(saved);
      setStage("receipt");
      setSlipUploaded(!!saved.slip_url); // ถ้ามี slip_url แปลว่าอัปโหลดแล้ว → ซ่อนส่วนชำระเงินด้วย
    }
  }, []);
  ...
  /**
   * ขั้นที่ 1: อัปโหลดสลิป + บันทึกคำสั่งซื้อ → เสร็จแล้วซ่อนอินพุต/ปุ่มอัปโหลด และซ่อน "วิธีการชำระเงิน (ธนาคาร/QR)"
   * จากนั้นแสดงเฉพาะปุ่ม "ยืนยันการชำระเงิน"
   */
  const handleSubmitSlipAndSave = async () => {
    ...
    try {
      ...
      // ✅ ซ่อนอินพุต/ปุ่มอัปโหลด + ซ่อนส่วนวิธีชำระเงิน และโชว์ปุ่มยืนยันอย่างเดียว
  ...
  /**
   * ขั้นที่ 2: กดยืนยัน → เปลี่ยนสถานะเป็น CONFIRMED แล้ว redirect ไป /waiting
   * ปุ่มนี้จะแสดงเฉพาะหลังจากอัปโหลดสลิปสำเร็จเท่านั้น
   */
          ...
          <div className="col-12">
            <div className="card shadow-sm">
              <div className="card-body">
                ...
                {/* ===== วิธีการชำระเงิน (ซ่อนทั้งหมดหลังอัปโหลดสลิปสำเร็จ) ===== */}
                {!slipUploaded && (
                  <div className="row g-4 mt-1">
                    {/* ซ้าย: รายละเอียดบัญชีโอน */}
                    ...
                    {/* ขวา: QR พร้อมเพย์ + ปุ่มดาวน์โหลด JPG */}
                    <div className="col-md-6">
                ...
                {/* ===== จบส่วนวิธีการชำระเงิน ===== */}

                {/* ===== คอนโทรลด้านล่าง: อัปโหลด → แล้วค่อยยืนยัน ===== */}
                <div className="row g-3 align-items-center mt-4">
                  {/* ก่อนอัปโหลด: แสดง input + ปุ่มอัปโหลด/ยกเลิก */}
                  {!slipUploaded && (
                    <>
                  ...
                  {/* หลังอัปโหลด: ซ่อน input/ปุ่มอัปโหลด และซ่อน “วิธีการชำระเงิน” ทั้งหมด → โชว์ปุ่มยืนยันอย่างเดียว */}
                  {slipUploaded && (
